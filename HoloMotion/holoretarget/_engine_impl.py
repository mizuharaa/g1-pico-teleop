...
# LOCAL PATCH (2026-08-07): pinned host memory is CUDA-only; gate it so the
# dormant CPU path can run on a GPU-less machine. No-op when CUDA is present.
def _pinned_ok() -> bool:
    try:
        import warp as _wp
        return bool(_wp.is_cuda_available())
    except Exception:
        return False


_PINNED_OK = _pinned_ok()
...
def pico_body_poses_to_xrobot_arrays(body_poses: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    # LOCAL PATCH R_y180: post-multiply by R_y180=(0,0,1,0) to match the GPU kernel
    quats_wxyz = quats_wxyz[:, [2, 3, 0, 1]] * np.array([-1.0, -1.0, 1.0, 1.0])
...
def pico_body_poses_to_xrobot_arrays32(body_poses: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    ...
    # LOCAL PATCH R_y180
    quats_wxyz = quats_wxyz[:, [2, 3, 0, 1]] * np.array([-1.0, -1.0, 1.0, 1.0], dtype=np.float32)
    return positions, quats_wxyz
...
class HoloNewtonG1Retargeter:
    ...
    def _targets_from_pico_body_poses_direct_fused(self, body_poses: np.ndarray) -> np.ndarray:
        ...
        # LOCAL PATCH R_y180
        q_holoretarget[:] = q_holoretarget[:, [2, 3, 0, 1]]
        q_holoretarget[:, :2] *= -1.0
