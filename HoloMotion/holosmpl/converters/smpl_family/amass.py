...
AMASS_SOURCE_COORDINATE_FRAME = "amass_smpl_default_z_up_meter"
AMASS_COORDINATE_TRANSFORM = "identity"
AMASS_FPS_FIELDS = ("mocap_frame_rate", "mocap_framerate", "fps")
AMASS_MOTION_FIELD_GROUPS = (
    ("trans",),
    AMASS_FPS_FIELDS,
    ("root_orient", "poses"),
    ("pose_body", "poses"),
...
def classify_amass_source(source_path: str | Path) -> dict[str, Any]:
    """Classify whether one AMASS NPZ is convertible before full conversion."""
    ...
    missing_groups = [
        list(group)
        for group in AMASS_MOTION_FIELD_GROUPS
    ...
    has_any_motion = any(
        name in source_fields
        for name in ("trans", "root_orient", "pose_body", "poses", *AMASS_FPS_FIELDS)
    )
    if not has_any_motion and set(source_fields).issubset(marker_like_fields):
        status = "unsupported_marker_only_or_stagei"
    else:
...
def convert_amass_sample(
    source_path: str | Path,
    *,
    input_root: str | Path,
    target_fps: float,
) -> dict[str, Any]:
    """Read one AMASS raw NPZ and return standardized canonical arrays."""
    ...
    with np.load(source_path, allow_pickle=True) as data:
        source_fields = sorted(data.files)
        root_orient, root_source = _load_root_orient(data)
        pose_body, pose_source = _load_pose_body(data)
    ...
    metadata = {
        ...
        "source_fields": source_fields,
        "root_orient_source": root_source,
        "pose_body_source": pose_source,
        ...
        "source_coordinate_system": AMASS_SOURCE_COORDINATE_FRAME,
        "canonical_coordinate_system": CANONICAL_COORDINATE_FRAME,
        "coordinate_transform": AMASS_COORDINATE_TRANSFORM,
...
def _required_array(data: Any, name: str) -> Any:
    if name not in data.files:
        raise KeyError(f"missing required AMASS field: {name}")
    return data[name]


def _load_root_orient(data: Any) -> tuple[Any, str]:
    if "root_orient" in data.files:
        return data["root_orient"], "root_orient"
...
def _load_pose_body(data: Any) -> tuple[Any, str]:
    if "pose_body" in data.files:
        ...
        return pose_body, "pose_body"
    poses = _required_array(data, "poses")
    if poses.ndim != 2 or poses.shape[1] < 66:
        raise ValueError(f"poses must be [T,D>=66] to derive AMASS body pose, got {poses.shape}")
    return poses[:, 3:66], "poses[:, 3:66]"
...
def _load_source_fps(data: Any) -> float:
    for name in ("mocap_frame_rate", "mocap_framerate", "fps"):
