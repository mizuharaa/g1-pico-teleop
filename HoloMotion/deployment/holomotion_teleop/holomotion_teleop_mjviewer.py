...
class ReferenceReceiver:
    ...
    def refresh(self) -> None:
        """Tear down and rebuild the SUB socket.

        Forces a clean reconnect when the publisher has gone away and come
        back (node restart, headset drop) instead of trusting a possibly
        dead transport.
        """
        try:
    ...
    def recv_latest(self, timeout_ms: int) -> Dict[str, np.ndarray] | None:
        try:
            ...
            print(f"[WARN] ZMQ recv failed ({exc}); rebuilding socket")
            self.refresh()
            return None
...
class ReferenceMujocoViewer:
    ...
    def __init__(self, mjcf_path: str | Path, viewer_fps: float, camera_follow: bool):
        ...
        self._orig_rgba = self.model.geom_rgba.copy()
        self._ghost = False

    def set_ghost(self, on: bool) -> None:
        """Render the robot translucent while the stream is stale/lost."""
        if on == self._ghost:
            return
        self._ghost = on
        if on:
            self.model.geom_rgba[:, 3] = self._orig_rgba[:, 3] * 0.25
        else:
            self.model.geom_rgba[:] = self._orig_rgba
...
def parse_args():
    ...
    parser.add_argument("--stale-s", type=float, default=1.0, help="Seconds without packets before the stream counts as stalled (robot ghosts)")
    parser.add_argument("--refresh-s", type=float, default=8.0, help="Rebuild the SUB socket after this many seconds without packets (0 disables)")
...
def main():
    ...
    viewer = None
    default_root_pos = np.array([0.0, 0.0, 0.793], dtype=np.float32)
    default_root_rot = np.array([1.0, 0.0, 0.0, 0.0], dtype=np.float32)
    default_dof_pos = np.zeros(DOF_DIM, dtype=np.float32)
    ...
    if not args.dry_run:
        ...
        default_root_pos = viewer.data.qpos[:3].copy().astype(np.float32)
        default_root_rot = viewer.data.qpos[3:7].copy().astype(np.float32)
        default_dof_pos = viewer.data.qpos[7 : 7 + DOF_DIM].copy().astype(np.float32)

    latest_root_pos = default_root_pos.copy()
    latest_root_rot = default_root_rot.copy()
    latest_dof_pos = default_dof_pos.copy()

    recv_count = 0
    last_recv_time = None
    recv_freq_log: list[float] = []

    # Stream-state machine: waiting (never seen data) -> live -> stale.
    # stale ghosts the robot so a frozen pose is visibly a DEAD STREAM, and
    # after --refresh-s the SUB socket is rebuilt to re-detect the publisher.
    stream_state = "waiting"
    last_socket_refresh = time.time()
    print(f"[stream] waiting for publisher at {args.uri} ...")
    ...
    try:
        while True:
            try:
                ...
                print(f"[WARN] dropped undecodable packet: {exc}")
                decoded = None
            if decoded is not None:
                ...
                if reference_qpos is None:
                    ...
                    try:
                        ...
                        print(f"[WARN] dropped malformed reference_qpos: {exc}")
                        continue

                    recv_count += 1
                    now = time.time()
                    if stream_state != "live":
                        print(f"[stream] LIVE (state was: {stream_state})")
                        stream_state = "live"
                        if viewer is not None:
                            viewer.set_ghost(False)
                ...
                if (
                    stream_state == "live"
                    and last_recv_time is not None
                    and now - last_recv_time > args.stale_s
                ):
                    stream_state = "stale"
                    print(
                        f"[stream] STALLED — no packets for {args.stale_s:.1f}s; "
                        "resetting to default standing pose (ghosted), matching "
                        "the robot-side failsafe"
                    )
                    # Mirror the real failsafe: the robot returns to its
                    # default standing pose on stream loss, so show that —
                    # not a frozen mid-motion pose.
                    latest_root_pos = default_root_pos.copy()
                    latest_root_rot = default_root_rot.copy()
                    latest_dof_pos = default_dof_pos.copy()
                    if viewer is not None:
                        viewer.set_ghost(True)
                if (
                    args.refresh_s > 0
                    and stream_state != "live"
                    and now - last_socket_refresh > args.refresh_s
                ):
                    print("[stream] rebuilding subscriber socket to re-detect publisher")
                    receiver.refresh()
                    last_socket_refresh = now
