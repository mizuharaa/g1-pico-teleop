...
class PicoReader:
    def __init__(self) -> None:
        ...
        # zombie-frame filter: after sleep/reconnect the PC Service can hand
        # one cached frame; demand an actually-advancing stream to resume.
        self._resume_gate = StreamResumeGate()
    ...
    def _run(self) -> None:
        ...
        # LOCAL PATCH (2026-08-11): while body data is missing, use the head
        # pose (which streams whenever the app is REALLY connected, in any
        # Mode) to tell apart the three failure states — so the operator gets
        # the actual fix instead of an endless "waiting for body data".
        last_head_check = 0.0
        prev_head = None
        while not self._stop.is_set():
            if not xrt.is_body_data_available():
                now = time.time()
                if now - last_head_check >= 5.0:
                    last_head_check = now
                    try:
                        head = np.asarray(xrt.get_headset_pose(), dtype=np.float32)
                    except Exception:
                        head = None
                    if (
                        head is not None
                        and head.size >= 7
                        and np.any(np.abs(head) > 1e-6)
                        and (prev_head is None or np.any(head != prev_head))
                    ):
                        print(
                            "[PicoReader] head_pose_ok — app IS connected but "
                            "sends no body data: in the app set Mode=Full-body "
                            "(it resets on every reconnect) and check 3 trackers lit"
                        )
                    else:
                        print(
                            "[PicoReader] head_pose_dead — no live headset link: "
                            "app not connected to THIS PC (use 192.168.123.2) "
                            "or the PC Service is wedged (post-sleep)"
                        )
                    prev_head = head
                time.sleep(0.001)
                continue
            ...
            if prev_stamp_ns is not None and stamp_ns == prev_stamp_ns:
                # LOCAL PATCH (2026-08-11): SDK latch bug — after headset
                # sleep/disconnect is_body_data_available() STAYS true while
                # timestamps freeze, so this branch spins silently forever.
                ...
                if now_frozen - getattr(self, "_frozen_report_t", 0.0) >= 5.0:
                    ...
                    try:
                        head = np.asarray(
                            xrt.get_headset_pose(), dtype=np.float32
                        )
                    except Exception:
                        head = None
                    prev_head = getattr(self, "_frozen_prev_head", None)
                    if (
                        head is not None
                        and head.size >= 7
                        and np.any(np.abs(head) > 1e-6)
                        ...
                        print(
                            "[PicoReader] head_pose_ok — stream timestamps "
                            "FROZEN (SDK latch after sleep) but link is live: "
                            "re-tick Send + Mode=Full-body in the app",
                            flush=True,
                        )
                    else:
                        print(
                            "[PicoReader] head_pose_dead — stream frozen and "
                            "no live link (post-sleep wedge)",
                            flush=True,
                        )
                    self._frozen_prev_head = head
                time.sleep(0.000001)
                continue
            ...
            if not self._resume_gate.accept():
                continue  # swallowed as a resume probe (zombie-frame filter)
...
class HoloRetargetTeleopNode:
        ...
        try:
            ...
            if self.latest_sample is not None:
                ...
                if source_age <= self._source_timeout_sec:
                    ...
                    reference_qpos = None
                    # stream gap: drop the EMA anchor so resume doesn't blend
                    # a minutes-old pose into the first fresh frame (phantom
                    # whole-body sweep at the exact moment tracking re-enters)
    ...
    def _smooth_reference(self, qpos: np.ndarray) -> np.ndarray:
        ...
        the 91->50 Hz aliasing jitter (±45% reference-velocity oscillation
        even with a smooth operator — measured in the 08-11 audit). alpha =
        fraction of the PREVIOUS frame kept; 0 (default) = off. Root quat
        uses sign-corrected nlerp; adds ~alpha/(1-alpha) frames of latency
        (0.4 -> ~13 ms), well under the perception threshold."""
        alpha = getattr(self, "_ref_smooth_alpha", None)
        if alpha is None:
            try:
                alpha = float(os.environ.get("HOLOTELEOP_REF_SMOOTH", "0"))
            except ValueError:
                alpha = 0.0
            self._ref_smooth_alpha = alpha
        if alpha <= 0.0:
            return qpos
        alpha = min(alpha, 0.95)
        qpos = np.asarray(qpos, dtype=np.float32)
        prev = getattr(self, "_ref_smooth_prev", None)
        if prev is None or prev.shape != qpos.shape:
            self._ref_smooth_prev = qpos.copy()
            return qpos
        out = qpos.copy()
        out[0:3] = alpha * prev[0:3] + (1.0 - alpha) * qpos[0:3]
        out[7:] = alpha * prev[7:] + (1.0 - alpha) * qpos[7:]
        q_prev = prev[3:7]
        q_new = qpos[3:7]
        if float(np.dot(q_prev, q_new)) < 0.0:
            q_new = -q_new
        q_mix = alpha * q_prev + (1.0 - alpha) * q_new
        norm = float(np.linalg.norm(q_mix))
        out[3:7] = q_mix / norm if norm > 1e-8 else qpos[3:7]
        self._ref_smooth_prev = out.copy()
        return out
...
def init_xrt(start_service: bool = True) -> None:
    ...
    # LOCAL PATCH (2026-08-11): this pre-loop runs BEFORE PicoReader exists,
    # so the link diagnostic must live HERE. Head pose streams whenever the
    # app is genuinely connected (any Mode) — use it to tell the operator
    # exactly which of the three failure states they are in.
    waited = 0
    prev_head = None
    while not xrt.is_body_data_available():
        if waited % 5 == 0:
            ...
            if (
                ...
                print(
                    "[PicoReader] head_pose_ok — app IS connected; no body "
                    "data means Mode!=Full-body (resets on every reconnect!) "
                    "or trackers not on/lit",
                    flush=True,
                )
            else:
                print(
                    "[PicoReader] head_pose_dead — no live headset link "
                    "(app not connected to 192.168.123.2, or service wedged)",
                    flush=True,
                )
            prev_head = head
