...
class LocalPicoRetargetSource:
    ...
    def step(self):
        ...
        self._retarget_errors = 0
        if qpos is None:
            # Guard suppressed the reference (sustained impossible input).
            # Returning None starves the policy's reference queue, and its
            # data-age failsafe switches back to velocity mode (default
            # standing pose) — the recoverable state.
            self._suppressed_count += 1
            if self._suppressed_count == 1 or self._suppressed_count % 100 == 0:
                self.logger.warn(
                    "[Retarget] reference SUPPRESSED by guard "
                    f"(x{self._suppressed_count}) — policy will fall back "
                    "to the default standing pose"
                )
            return None
