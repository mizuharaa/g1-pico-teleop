...
class PolicyRuntimeState:
    ...
    motion_soft_start_t0: float | None = None
    mode_blend_t0: float | None = None
    mode_blend_from_real: np.ndarray | None = None
...
class PolicyRuntime:
    ...
    def __init__(self, port: Any, num_actions: int = 29):
        try:
            self._soft_start_s = float(
                os.environ.get("HOLOMOTION_SOFT_START_S", "2.5")
            )
        except ValueError:
            self._soft_start_s = 2.5
        # LOCAL PATCH (2026-08-11): tracking->velocity re-entry blend (the
        # mirror of the B-press soft-start). Without it, Y-press or a stale
        # -data fallback snaps the commanded target from the tracked pose to
        # the velocity default pose in ONE control step.
        try:
            self._mode_blend_s = float(
                os.environ.get("HOLOMOTION_MODE_BLEND_S", "1.5")
            )
        except ValueError:
            self._mode_blend_s = 1.5
        # LOCAL PATCH (2026-08-11): per-joint target slew limiter (rad/s).
        # 0 = OFF (robot default). Caps the measured 0.5 rad/20 ms ankle
        # spikes in raw policy output. Applied post-store like the blend, so
        # the policy's own action history is untouched.
        try:
            self._target_slew_rad_s = float(
                os.environ.get("HOLOMOTION_TARGET_SLEW_RAD_S", "0")
            )
        except ValueError:
            self._target_slew_rad_s = 0.0
        self._slew_prev_target: np.ndarray | None = None
        self._slew_prev_t: float | None = None
        self.port = port
    ...
    def set_robot_state(self, robot_state: str) -> None:
        if robot_state == "MOVE_TO_DEFAULT":
            ...
            self._slew_prev_target = None
            self._slew_prev_t = None
    ...
    def switch_to_velocity_mode(self, reason: str = "") -> None:
        # LOCAL PATCH (2026-08-11): capture the last commanded pose so the
        # velocity policy's default-pose target can be blended in over
        # HOLOMOTION_MODE_BLEND_S instead of snapping in one step.
        if self.state.target_dof_pos_real is not None:
            self.state.mode_blend_from_real = np.array(
                self.state.target_dof_pos_real, dtype=np.float32, copy=True
            )
            self.state.mode_blend_t0 = time.time()
        self.state.current_policy_mode = "velocity"
        self.state.motion_uses_vr_reference = False
        self.state.motion_in_progress = False
        obs_eval = self._obs_evaluator()
    ...
    def switch_to_motion_mode(self) -> bool:
        ...
        # LOCAL PATCH (2026-08-10): soft-start — the operator's pose rarely
        # matches the robot's default pose at B; ramp the action authority
        # from zero so the robot GLIDES to the tracked pose instead of
        # lunging at it.
        self.state.motion_soft_start_t0 = time.time()
        # LOCAL PATCH (2026-08-11 rev2): ALSO blend into tracking from the
        # last commanded pose. Soft-start alone still snaps to the motion
        # model's default pose at entry (waist_pitch differs +0.107 rad from
        # the velocity default) — a forward-lean snap that lunges the robot
        # at every B-press. The real-space blend makes the commanded pose
        # continuous across the switch; soft-start still ramps the actions.
        if self.state.target_dof_pos_real is not None:
            self.state.mode_blend_from_real = np.array(
                self.state.target_dof_pos_real, dtype=np.float32, copy=True
            )
    ...
    def reset_counters(self) -> None:
        ...
        # slew anchor must not survive a mode switch or pause: a stale prev
        # drags the command from an old pose at slew*dt per tick ("robot
        # crawls / ignores the operator"). Every mode entry calls here.
        self._slew_prev_target = None
        self._slew_prev_t = None
    ...
    def apply_policy_output(
        ...
        *,
        ...
        # LOCAL PATCH (2026-08-10): soft-start ramp. target = default +
        # alpha * (actions * scale); alpha smoothsteps 0 -> 1 over
        # HOLOMOTION_SOFT_START_S after entering motion mode, so the robot
        # starts AT its default pose and eases into tracking the operator.
        if is_motion and self.state.motion_soft_start_t0 is not None:
            if self._soft_start_s <= 0.0:
                self.state.motion_soft_start_t0 = None
            else:
                t = (
                    time.time() - self.state.motion_soft_start_t0
                ) / self._soft_start_s
                if t >= 1.0:
                    self.state.motion_soft_start_t0 = None
                else:
                    t = max(t, 0.0)
                    alpha = t * t * (3.0 - 2.0 * t)
                    self.state.target_dof_pos_onnx *= alpha
        np.add(
            self.state.target_dof_pos_onnx,
            default_angles,
            out=self.state.target_dof_pos_onnx,
        ...
        # LOCAL PATCH (2026-08-11): mode-transition blend (real-joint order,
        # so it is valid across models with different onnx orderings). Blends
        # the previous mode's last commanded pose into the new target with a
        # smoothstep over HOLOMOTION_MODE_BLEND_S. Applied AFTER actions_onnx
        # is stored, so the policy's own action history stays untouched —
        # same invariant as the motion soft-start above.
        if self.state.mode_blend_t0 is not None:
            prev = self.state.mode_blend_from_real
            if (
                self._mode_blend_s <= 0.0
                or prev is None
                or prev.shape != target_real.shape
            ):
                self.state.mode_blend_t0 = None
                self.state.mode_blend_from_real = None
            else:
                t = (time.time() - self.state.mode_blend_t0) / self._mode_blend_s
                if t >= 1.0:
                    self.state.mode_blend_t0 = None
                    self.state.mode_blend_from_real = None
                else:
                    t = max(t, 0.0)
                    beta = t * t * (3.0 - 2.0 * t)
                    np.multiply(target_real, beta, out=target_real)
                    target_real += (np.float32(1.0) - np.float32(beta)) * prev
        # LOCAL PATCH (2026-08-11): slew limit AFTER blend, in real order.
        if self._target_slew_rad_s > 0.0:
            prev = self._slew_prev_target
            now = time.time()
            if prev is not None and prev.shape == target_real.shape:
                # measured elapsed, clamped to [dt, 4*dt]: at overrun rates
                # (e.g. 30 Hz actual vs 50 Hz nominal) a fixed dt silently
                # tightens the limit by the overrun factor.
                dt_eff = self.port.dt
                if self._slew_prev_t is not None:
                    dt_eff = min(max(now - self._slew_prev_t, self.port.dt),
                                 4.0 * self.port.dt)
                max_step = np.float32(self._target_slew_rad_s * dt_eff)
                np.clip(
                    target_real,
                    prev - max_step,
                    prev + max_step,
                ...
                np.copyto(prev, target_real)
            else:
                self._slew_prev_target = target_real.copy()
            self._slew_prev_t = now
