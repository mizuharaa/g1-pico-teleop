 ...
 **/
 ...
 private:
   ...
   void Control() {
     ...
     // LOCAL PATCH (2026-08-11): bare L1 used to force instant ZERO_TORQUE
     // from ANY state (kp=kd=0 = free-fall — strictly worse than damping on
     // a standing robot). Bare L1 now routes to the damped EMERGENCY_STOP
     // sequence (2 s kd=10, then disable), same as Select. The deliberate
     // chord L1+L2 keeps the original instant zero-torque for bench use
     // (robot on a stand / fully supported).
     if (remote_controller.button[KeyMap::L1] == 1 &&
         current_state_ != RobotState::ZERO_TORQUE) {
         if (remote_controller.button[KeyMap::L2] == 1) {
             RCLCPP_WARN(this->get_logger(),
                 "L1+L2 chord: switching to ZERO_TORQUE (instant limp — "
                 "robot must be supported)");
             current_state_ = RobotState::ZERO_TORQUE;
             publishRobotState();
         } else {
             RCLCPP_WARN(this->get_logger(),
                 "L1 pressed: routing to damped EMERGENCY_STOP (instant "
                 "zero-torque on bare L1 disabled by local patch; use "
                 "L1+L2 if the robot is supported)");
             current_state_ = RobotState::EMERGENCY_STOP;
             should_shutdown_ = true;
             publishRobotState();
             return;
